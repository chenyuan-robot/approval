      ...
      >
        ...
        >
          ...
          <template v-else>
            ...
            <view v-else class="uni-select__input-text uni-select__input-placeholder" :class="'align-' + align">{{
              typePlaceholder
            }}</view>
          </template>
          <view key="clear-button" v-if="!hideRight && shouldShowClear && clear && !disabled" @click.stop="clearVal">
            <UiIcons type="clear" color="#c0c4cc" size="24" />
          </view>
          <view key="arrow-button" v-else-if="!hideRight">
            <UiIcons :type="showSelector ? 'top' : 'bottom'" size="14" color="#999" />
          </view>
        </view>
            ...
            <template v-if="slotOption">
              <view v-for="(itemData, index) in mixinDatacomResData" :key="index" @click="change(itemData)">
                <slot
                  ...
                  :item="itemData"
                  :itemSelected="
                    multiple ? getCurrentValues().includes(itemData.value) : getCurrentValues() == itemData.value
                  "
                ></slot>
              </view>
            </template>
...
<script>
import UiIcons from '@/components/ui/ui-icons/components/uni-icons/uni-icons.vue'
/**
 * DataChecklist 数据选择器
